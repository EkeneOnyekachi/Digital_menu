...
AUTH0_DOMAIN = "ekene001.us.auth0.com"
ALGORITHMS = ["RS256"]
API_AUDIENCE = "Coffe_API"
...
def get_token_auth_header():
    ...
    if parts[0].lower() != "bearer":
        ...
        raise AuthError(
            {"message": "Invalid_Header", "description": "Token not found"}, 401
        )
    elif len(parts) > 2:
        raise AuthError(
            {
...
def check_permissions(permission, payload):
    ...
    if permission not in payload["permissions"]:
        raise AuthError(
            {"message": "Invalid_assertion", "description": "permissions not found"},
            403,
        )
    return True
